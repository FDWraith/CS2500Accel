...
;; Food is one of NO-FOOD or (make-food x y)
...
;;                          ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;; ListTurkey -> Number
;; outputs the largest size of a Turkey in the ListTurkey
...
(define (max-size.v0 lt)
  (cond
    [(empty? (rest lt)) (return-turkey-size (first lt))]
    [else (max (return-turkey-size (first lt)) (max-size (rest lt)))]))
|#
...
#|
(define (parse-thru-list.v0 lt img)
  (cond
    [(empty? (rest lt)) (render-turkey (first lt) img)]
    [else (render-turkey (first lt) (parse-thru-list (rest lt) img))]))
|#
...
;;                                  ABSTRACTION OF parse-thru-list
...
(define (update-food g)
  (cond
    ...
    [else (local (;; Posn [List-of Turkey] Number -> Boolean
                  ...
                  #|
                  (define (listturkey-close?.v0 p lt delta)
                    (cond
                      [(empty? (rest lt)) (close? (turkey-loc (first lt)) p delta)]
                      [else (or (close? (turkey-loc (first lt)) p delta)
                                (listturkey-close? p (rest lt) delta))]))
                  |#
                  
                  ;; Posn [List-of Turkey] Number -> Boolean
                  ;; takes a list of turkeys and checks if each of them are close to the food
...
;;                         ~~~~~~~~~~ HELPER FUNCTION ~~~~~~~~~~~~~
;;                         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;; Food ListTurkey -> ListTurkey
;; Contructs a ListTurkey in which each Turkey has its updated size and position
...
(define (update-listturkey.v0 f lt)
  (cond
    [(empty? (rest lt)) (cons (update-turkey f (first lt)) '())]
    [else (cons (update-turkey f (first lt)) (update-listturkey f (rest lt)))]))
|#
...
;;                                  ABSTRACTION OF update-listturkey

;; Food [List-of Turkey] -> [List-of Turkey]
