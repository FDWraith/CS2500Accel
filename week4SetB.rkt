...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname week4SetB) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
;; Turkey Turkey  Image-> Image
;; adding turkeys to given image
(check-expect (render-turkeys t-player5 t-ai5 BACKG)
              (place-images
               (list (circle (turkey-size t-player5) "solid" PLAYER-COLOR)
                     (circle (turkey-size t-ai5) "solid" AI-COLOR))
               (list (turkey-loc t-player5)
                     (turkey-loc t-ai5))
               BACKG))
(define (render-turkeys t1 t2 im)
  (place-images
   (list (circle (turkey-size t1) "solid" (turkey-color t1))
         (circle (turkey-size t2) "solid" (turkey-color t2)))
   (list (turkey-loc t1)
         (turkey-loc t2))
   im))
